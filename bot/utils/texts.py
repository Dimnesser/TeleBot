...
DEPOSIT_CONFIRM_TEXT = (
    ...
    "Предметы: {items}\n"
    "Код: {promo}\n"
    "Итого к зачислению: <b>{total} B</b>{bonus}\n\n"
    "После проверки модератором баланс будет пополнен."
)
DEPOSIT_PROMO_PROMPT = (
    "Есть код? Любой промокод, партнёрский или реферальный код даёт бонус к зачислению. "
    "Отправь код или нажми «Пропустить»:"
)
...
# Очередь пополнений и выводов — одни и те же сообщения игроку.
QUEUE_FIRST_TEXT = (
    "📥 {kind} №{request_id} принята — ты <b>1-й в очереди</b>, заявка уже в работе.\n"
    "Модератор кинет трейд на ник <b>{nickname}</b>."
)
QUEUE_WAIT_TEXT = (
    "📥 {kind} №{request_id} принята — ты <b>{position}-й в очереди</b>.\n"
    "Когда подойдёт твоя очередь, пришлю сообщение."
)
QUEUE_TURN_TEXT = (
    "🔔 <b>Твоя очередь настала!</b> {kind} №{request_id} в работе — "
    "модератор кинет трейд на ник <b>{nickname}</b>."
)
QUEUE_CANCELLED_TEXT = "↩️ {kind} №{request_id} отменена."
...
STARS_AMOUNT_INVALID = "Введи целое число Stars — от 1 и больше."
...
FAQ_ENTRIES = [
    ...
    ("Как вывести брейнрота?", "В Mini App: инвентарь → «Вывести» (или «Обменять» на других из стока той же цены — разницу бот доплатит в B). Встаёшь в очередь, когда она дойдёт — админ кинет трейд в игре."),
...
# --- поддержка ---
SUPPORT_WELCOME = (
    "🆘 <b>Поддержка BrainCore</b>\n\n"
    "Напиши вопрос одним сообщением — можно приложить скрин или видео. "
    "Оператор ответит прямо здесь, в этом чате.\n\n"
    "Или глянь частые вопросы — ответ придёт сразу 👇"
)
SUPPORT_SENT = "✅ Сообщение передано в поддержку — ответ придёт сюда. Можешь дописать ещё."
SUPPORT_FAILED = "⚠️ Не получилось связаться с поддержкой, попробуй чуть позже."
SUPPORT_SLOW_DOWN = "⏳ Не так быстро — подожди секунду и отправь снова."
SUPPORT_CLOSED = "Обращение завершено. Если что — жми /support или просто напиши сюда."
SUPPORT_ADMIN_HELLO = (
    "👋 Ты админ. Обращения игроков будут приходить сюда.\n"
    "Чтобы ответить — сделай <b>реплай</b> (ответить) на сообщение игрока."
)
